...
//
// Comets (SPEC-EVENTS-001 added 핼리 혜성) are excluded for a reason of the SHARED
// BASIS above, not of taste: at e=0.967 Halley swings between roughly 0.6 AU and
// 35 AU, so there is no single fast-cruise duration to speak. Giotto reached it in
// eight months only by intercepting it near perihelion; quoting that against its
// 17.8 AU semi-major axis would tell a child it is nearer than Jupiter. A body with
// no honest one-way number offers no rocket button, the same rule SizeCompare
// applies when no ratio lands within ±10%.
const INELIGIBLE_CATEGORIES = new Set(['comet']);

/**
 * Every body the rocket may fly to: sun-orbiting planets and dwarf planets except
 * Earth, plus Earth's Moon. Not the Sun, not stars, not comets (REQ-PLAY-201).
 * @returns {string[]}
 */
  ...
  const orbiting = Object.entries(PLANET_DATA)
    .filter(
      ([key, body]) =>
        ...
        !INELIGIBLE_CATEGORIES.has(body.category)
    )
    .map(([key]) => key);
