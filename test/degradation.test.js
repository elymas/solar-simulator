...
describe('FrameBudgetDegrader priority order (REQ-240, TASK-010)', () => {
  // 'belts' joined the head of the solar ladder in SPEC-EVENTS-001 REQ-EVT-204;
  // the bloom -> lod -> pixelRatio order below it is unchanged.
  it('degrades belts -> bloom -> lod -> pixelRatio, 30 consecutive over-budget frames each', () => {
  ...
  it('resets the over-budget streak on a single good frame', () => {
    ...
    for (let i = 0; i < 29; i++) expect(d.record(over())).toBeNull();
    expect(d.record(over())).toBe('belts'); // requires a fresh run of 30
  });
  ...
  it('recovers in reverse order after sustained good frames', () => {
    ...
    for (let i = 0; i < 30; i++) d.record(over()); // -> belts (level 1)
    for (let i = 0; i < 30; i++) d.record(over()); // -> bloom (level 2)
    expect(d.level).toBe(2);
    for (let i = 0; i < 59; i++) expect(d.record(under())).toBeNull();
    expect(d.record(under())).toBe('restore:bloom'); // last applied recovers first
